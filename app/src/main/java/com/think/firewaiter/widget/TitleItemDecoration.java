...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
  private List<PxTableInfo> mDatas;
  ...
  private static int COLOR_ITEM_DECORATION = Color.parseColor("#ff0fffff");
  ...
  //@formatter:off
  public TitleItemDecoration(Context context, List<PxTableInfo> datas) {
    super();
    mDatas = datas;
    mPaint = new Paint();
    mBounds = new Rect();
  ...
  private void drawTitleArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {//最先调用，绘制在最下层
    ...
    c.drawRect(0, bottom - mTitleHeight, right, bottom, mPaint);
...
/*
  ...
  int baseline = (getMeasuredHeight() - fontMetrics.bottom + fontMetrics.top) / 2 - fontMetrics.top;*/
    ...
    c.drawText(tag, 20, child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
  }
  ...
  public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
    ...
    //  //出现一个奇怪的bug，有时候child为空，所以将 child = parent.getChildAt(i)。
    ...
    //        //可与123行 c.drawRect 比较，只有bottom参数不一样，由于 child.getHeight() + child.getTop() < mTitleHeight，
    ...
    //  mPaint.setColor(COLOR_TITLE_FONT);
    //  mPaint.getTextBounds(tag, 0, tag.length(), mBounds);
    //  c.drawText(tag, 20, parent.getPaddingTop() + mTitleHeight - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
  ...
  public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
    ...
    mPaint.setColor(COLOR_ITEM_DECORATION);
    outRect.bottom = 2;
  }
}